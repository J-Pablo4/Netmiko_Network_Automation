...
for i in range(0, sh_output_length):
    if sh_output.find('transport input all') != -1 or sh_output.find('transport input telnet') != -1:
        telnet_is_up = True
...
if telnet_is_up:
    ...
    print("**Telnet has been Disabled**\n")
    time.sleep(1)
else:
...
if not banner_exist:
    ...
    sh_output = net_connect.send_command(f'banner motd # *****{message}***** #')
    print("**The Message of the Day has been Set Correctly**\n")
    time.sleep(1)
else:
    print("**The Message of the Day has been Set**\n")
    time.sleep(1)
# Loopback1
